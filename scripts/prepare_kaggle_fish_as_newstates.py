"""
prepare_kaggle_fish_as_newstates.py — Armed Struggle ch2 "NEW STATES 1923-63"
(the chapter Dave is listening to, 39.9% in) via the proven Fish S2 Pro pilot
harness (single-process, dual-T4, banked, gated). Free Kaggle only.

Chapter-specific glossary extends the repo lexicon for surface forms that occur
in this chapter: Fianna Fail, standalone Dail/Eireann, Sean, Eamon, IRA -> I-R-A.
"""
...
chapter_text = Path(r"C:\Users\Dave\AppData\Local\Temp\opencode\as_ch2_newstates.txt").read_text(encoding="utf-8")
...
for key in sorted((k for k in lex if not k.startswith("_")), key=len, reverse=True):
    chapter_text = chapter_text.replace(key, lex[key])
for key in sorted(chapter_glossary, key=len, reverse=True):
    chapter_text = chapter_text.replace(key, chapter_glossary[key])

stage_dir = root / "scratch" / "kaggle_fish_as_newstates" / "kernel"
stage_dir.mkdir(parents=True, exist_ok=True)

kernel_code = f'''#!/usr/bin/env python3
"""Fish S2 Pro: Armed Struggle ch2 'New States 1923-63'. Single-process, dual-T4, banked."""
...
for idx, s in enumerate(sents, 1):
    ...
    try:
        ...
        audio_secs += secs
        manifest.append({{"idx": idx, "sec": round(secs, 2), "ok": True}})
        if idx % 25 == 0 or idx == len(sents):
            el = time.time() - t0
            print(f"[{{idx}}/{{len(sents)}}] running RTF {{el / max(audio_secs, 1):.2f}}x ({{n_fail}} failed)", flush=True)
            (OUT / "manifest.json").write_text(json.dumps({{"elapsed": round(el, 1), "audio_secs": round(audio_secs, 1), "n_fail": n_fail, "done": idx, "sents": manifest}}, indent=1))
...
sf.write("/workspace/as_ch2_raw.wav", full, sr)
subprocess.run(["ffmpeg", "-y", "-loglevel", "error", "-i", "/workspace/as_ch2_raw.wav",
                "-af", "equalizer=f=220:width_type=o:width=1.2:g=1.0,highshelf=f=7500:gain=-2.0:width=1.0,loudnorm=I=-20:TP=-2:LRA=11",
                "-codec:a", "libmp3lame", "-b:a", "192k", str(OUT / "armed_struggle_ch2_newstates_cillian.mp3")], check=True)
a, sra = sf.read("/workspace/as_ch2_raw.wav", dtype="float32")
if a.ndim > 1:
    a = a.mean(axis=1)
...
if not ok:
    (OUT / "armed_struggle_ch2_newstates_cillian.mp3").rename(OUT / "armed_struggle_ch2_newstates_cillian_UNHEALTHY.mp3")
...
meta = {
    "id": "davedavedavedavenm/fish-as-newstates-cillian",
    "title": "fish-as-newstates-cillian",
