"""
prepare_kaggle_fish_atthe.py — render the orphan fragment 'At the' (fixes the
dropped fragment before 'army's March ... General Army Convention' in AS ch2 v2).
Tries several spellings; banks the first health-gated pass as atthe.wav.
"""
...
stage_dir = root / "scratch" / "kaggle_fish_atthe" / "kernel"
...
for cand in CANDIDATES:
    try:
        ...
        if ok and got is None:
            sf.write(str(OUT / "atthe.wav"), wav, codec.sample_rate)
            got = cand
...
meta = {
    "id": "davedavedavedavenm/fish-atthe-fix",
    "title": "fish-atthe-fix",
