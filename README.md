...
| `LIGHTNING_USERNAME` / `LIGHTNING_API_KEY` | **Paid** lane credentials — reachable only by naming the lane explicitly; the key lives in `.secrets/lightning_api_key` + the settings DB, never in the repo. **Not usable inside the app image**: `lightning-sdk` caps `urllib3<=2.5.0` while the Vast CLI needs `>=2.7.0` (measured across all versions of both, 2026-09-25), so it is deliberately not installed — the lane status reports `lightning-sdk not installed in this environment` and a named render is refused at start-up. Separate venv required. |
