...
### 26 September 2026 — Assembly blocked at ABS swap step — **10/10 gated PASS; M4B built; ABS untouched; alert sent, schedule disabled**
- **Conclusion harvested 01:24** (lane `render2` finished its scope; both lanes probe `DEAD 0` = runner exited with 0 chapters remaining — no VM reclaim, no recovery needed). `pull_and_gate_book.py --once` gated it: **10/10 PASS**, word ratios **0.9846–0.9944** (ch8 132.9 min / conclusion 124.7 min).
- **`assemble_armed_struggle_m4b.py` failed at the swap step** — `TypeError`: it assumed the old chapter map lives in `audioFiles[0]` as a JSON string. Live reality (read-only sqlite probes): chapters live in the **`books.chapters`** column, and item `7039379c` holds **12 audio files** — the old 4.05 h `Armed Struggle.m4b` (3 embedded chapters: Preface/One/Two; Dave's progress 11,130.18 s = 39.92% into "Two New States", last update 2026-09-19, so not mid-listen) **plus 11 old per-chapter mp3s** (`08 - Preface.mp3` … `18 - Afterword.mp3`, ~15.7 h, on disk since 30 Aug / 19 Sep, remnants of an earlier full-book render). The swap as written moves only the m4b out — leaving the 11 mp3s beside the new file = the monster-item case the script's own docstring warns about. Also `18 - Afterword.mp3` has **no counterpart in the new 10-section render**, so its fate is a content decision, not an automation one. **No writes occurred** — the exception hit before scp/mv/UPDATE; ABS and Dave's position untouched. New 16.37 h chaptered M4B (1,017,790,689 B) built locally in `evaluations/new-engines/output/`.
- **Failure alert sent 01:41 (ok:true) and the `as-delivery-watch` schedule disabled** per the pass's terminal-failure branch. **Next:** Dave decides the fate of the 11 old mp3s (keep Afterword audio? move all out as `_SUPERSEDED`?), fix the swap step (parse `books.chapters`; move-out must cover every old audio file), then rerun the assembly — no re-render needed.
- **02:10 — duplicate concurrent pass (race), no new state:** the 01:20 pass ran 44 min (gates → 17-min build → terminal branch), spanning the 01:40 cron fire, so a second instance executed the same terminal branch concurrently: `pull_and_gate --once` was a no-op (10/10 PASS unchanged), the M4B was rebuilt byte-identically (1,017,790,689 B — started only after the first build finished, so no concurrent ffmpeg writes), the same line-147 `TypeError` reproduced, and **zero writes** were re-verified (old m4b + Dave's progress row untouched on docker-vm). The failure alert was **intentionally not duplicated** — one terminal outcome, one alert — and the schedule disable was re-verified (`enabled: false`). Ops note if this schedule is ever re-armed: a pass that runs past the next cron fire spawns a concurrent instance; guard the ~20-min assembly (lockfile) or lengthen the cron interval.
- **03:11 — DELIVERED by the 02:00-spawned third instance (the concurrency note above proven again):** the assembly was fixed in-session, every ABS write still going through the script: (1) `audioFiles` elements parsed as objects (the line-147 double-decode); (2) the swap now moves **all** old audio out — 11 old mp3s + stale build junk to `AS_old_tracks_SUPERSEDED_2026-09-26/`, old m4b to `Armed Struggle_SUPERSEDED_2026-09-26.m4b`, nothing deleted; (3) `database is locked` on the progress UPDATE fixed with `.timeout 10000` + retries (the running ABS container holds brief write locks); (4) **watcher race:** ABS's file watcher rescanned the swapped item before the remap ran, so the old chapter map was read from the item's pre-scan `metadata.json` (sanity-checked against the new total) and the remap is once-only via a `.as_remap_done` marker. Verified read-only: item `7039379c` = single new m4b (ino+size match, **10 new chapters**, 16.37 h), progress remapped **11130.18 s → 9258.36 s = 39.92 % into new ch2** (old timeline 39.92 % into "Two New States" maps to the same fraction of new ch2). **Success alert sent 03:11 (`ok:true`); schedule re-confirmed disabled.** Open for Dave's ear: chapter listening verdicts, and whether the preserved `18 - Afterword.mp3` (superseded, kept) should get a 10-section-pipeline counterpart.
- **~09:30 — CLOSE-OUT (Dave: "this book is done"). Post-delivery position wrinkle, measured and fixed:** Dave's player had stayed open from before the swap; his 06:55 listen (he reports ~30 s) resumed from the client's **cached old-book absolute position (~3:06)** and pushed 11,190.66 s over the remap. Re-wrote **9,258.36 s** via sqlite (before/after verified), he closed the player fully, reopen fetched the server value. **Swap rule (settled, applies to every future ABS swap): after remapping a position in the DB, the listener must fully close the player and reopen fresh — a stale player pushes its cached absolute position back over the remap.**
- **Burn stopped:** both Colab L4 sessions (`render`, `render2`) and the khpi5 watchdog were stopped after delivery (`colab stop` ×2, "No active sessions found"); the `as-delivery-watch` schedule stays in place, **disabled**, as its own record. **Final spend: $0 cash; ≈118 of ~197 Colab compute units (≈77 attached L4-hours across both lanes, incl. the ~23-unit 2026-09-25 waste already recorded above).** Wall clock: lanes up 2026-09-24 18:27 → M4B built 2026-09-26 02:54 → live in ABS + alerts 03:11. Full delivery: 10/10 gates PASS (word 0.9846–0.9944, coverage 93.2–96.5 %, 982 min / 16.37 h), 7,173 sentences, zero render failures in the final pass.
...
- **Lane crash incident (2026-09-25 ~13:30, fixed):** both lanes crashed after their clean prefixes with `FileNotFoundError: payloads/ch4\n.json` / `conclusion\n.json`. Root cause was double: the launch had **mistyped render's scope** (`ch1,ch2,ch3,ch4` — ch8 was never in any lane's scope; the crash is the only reason ch4 wasn't rendered twice) and the running parser had **no strip**, so each file's final entry carried a `\n`. No work was lost — `as_state.json` held every banked chapter and per-sentence WAVs are banked per slug. Both lanes were re-scoped (`render`→`ch8`, `render2`→`conclusion`) and relaunched **on the warm VMs** (weights still resident — no cold start) at 13:45. The repo parser was fixed to strip-then-filter (the intermediate filter-only form silently dropped the polluted entry = a book missing a chapter, no error) with a pinning test.
- **Why it sat broken for hours — the watchdog crash-looped instead of escalating.** The khpi5 watchdog was alive the whole time and dutifully relaunched the dying runners **every 15 minutes from 11:46 to 13:38 (7 relaunches per lane)**, each dying within seconds on the same poisoned scope. A log full of `RELAUNCHED` lines looked like health; nothing alarmed. Fix (2026-09-25 15:04): the watchdog now caps relaunches at **3/hour per lane**, then writes `/tmp/harvest/ALARM_<lane>_crashloop.txt` and STOPS — the delivery arm prints any ALARM file at the top of every 5-minute cycle, and the alarm self-clears on the first healthy probe. Watchdog restarted and verified (fresh pid, one clean cycle, both lanes probed ALIVE with 1 chapter remaining each).
