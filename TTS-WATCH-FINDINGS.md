...
- **Lanes status:** Kaggle weekly 30 h cap hit 23 Sep (fallback kernels staged; auto-retry loop left running); **Lightning free tier cannot start GPUs without a linked payment method** (400 PermissionDenied verified 24 Sep; T4_X_2 also silently fell back to CPU) — excluded; Modal remains vetoed (zero credits). Lightning key retained dormant: `.secrets/lightning_api_key`.
